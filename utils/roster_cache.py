    ...
    async def get_team_roster(self, team_id: str, team_name: str) -> List[Dict]:
        ...
        try:
            async with await self._rate_limited_get(url) as response:
                if response.status != 200:
                    logger.warning(f"Failed to fetch roster for {team_name}: HTTP {response.status}")
                    return []
...
def lookup_player_team_with_id(player_name: str, cache_file: str = None) -> Optional[Tuple[str, str]]:
    """
