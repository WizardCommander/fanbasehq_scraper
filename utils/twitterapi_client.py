...
from config.settings import (
    TWITTER_API_KEY, TWITTER_API_BASE_URL, TWITTER_API_TIMEOUT, 
    TWEETS_PER_PAGE, DEFAULT_RATE_LIMIT_DELAY
)
...
class TwitterAPIClient:
    ...
    async def search_tweets(
        ...
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=TWITTER_API_TIMEOUT)) as session:
            while pages_fetched < max_pages and len(tweets) < limit:
                try:
                    ...
                    async with session.get(url, headers=self.headers, params=params) as response:
                        ...
                        logger.info(f"Got {len(page_tweets)} tweets from page {pages_fetched + 1}")
        ...
        for account in accounts:
            ...
            for variation in player_variations:
                try:
                    ...
                    logger.info(f"Found {len(tweets)} tweets from {account} mentioning '{variation}'")
    ...
    def _convert_tweet_data(self, tweet_data: Dict) -> Optional[ScrapedTweet]:
        ...
        try:
            ...
            is_quote = any(ref.get('type') == 'quoted' for ref in tweet_data.get('referenced_tweets', []))
