...
from config.settings import DEFAULT_RATE_LIMIT_DELAY, MAX_RETRIES
...
class TwitterClient:
    ...
    async def setup(self):
        ...
        try:
            ...
            accounts = await self.api.pool.accounts_count()
            logger.info(f"Twitter accounts available: {accounts}")
            
            if accounts == 0:
                logger.warning("No Twitter accounts configured. You'll need to add accounts with: twscrape add_account")
    ...
    def _convert_tweet(self, tweet: Tweet) -> ScrapedTweet:
        ...
        return ScrapedTweet(
            ...
            author=tweet.user.displayname or tweet.user.username,
